...
async function findUserForLogin(email) {
  ...
  return users.find((user) => String(user.mail ?? '').trim().toLowerCase() === normalizedEmail) ?? null;
}

function isValidPassword(user, password) {
  return String(user?.contraseña ?? '').trim() === String(password ?? '').trim();
}
...
app.get('/health', async (_req, res) => {
  try {
    await countTableRows('Usuario');
    res.json({
      ok: true,
      service: 'comuniplus-backend',
      db: true,
      port,
      supabaseProject: getSupabaseProjectRef(supabaseUrl)
    });
  } catch (error) {
    res.status(500).json({
      ok: false,
      ...
      port,
      supabaseProject: getSupabaseProjectRef(supabaseUrl),
      message: 'No se pudo conectar correctamente con Supabase',
      detail: error.message,
      supabase: mapSupabaseError(error)
    });
  }
});
...
app.get('/api/debug/supabase', async (_req, res) => {
  try {
    const tables = ['Usuario', 'Comunidad', 'ComunidadUsuario', 'solicitudViaje', 'Viaje'];
    const counts = Object.fromEntries(
      await Promise.all(tables.map(async (table) => [table, await countTableRows(table)]))
    );

    res.json({
      ok: true,
      project: getSupabaseProjectRef(supabaseUrl),
      tables: counts
    });
  } catch (error) {
    res.status(500).json({
      ok: false,
      message: 'Error revisando tablas de Supabase',
      failedTable: error.tableName,
      detail: error.message,
      supabase: mapSupabaseError(error)
    });
  }
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    if (!user) {
      return res.status(401).json({ message: 'No encontré un usuario con ese mail en Supabase' });
    }

    if (!isValidPassword(user, password)) {
      return res.status(401).json({ message: 'Contraseña incorrecta para ese usuario' });
    }
...
app.listen(port, () => {
  ...
  console.log(`Supabase project: ${getSupabaseProjectRef(supabaseUrl)}`);
});
