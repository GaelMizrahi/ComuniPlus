...
export async function requestRide(req, res) {
  try {
    ...
    res.status(status).json({
      message: error.message || 'Error al solicitar viaje'
    });
  }
}
...
export async function offerRide(req, res) {
  try {
    const result = await rideService.offerRide(
      req.user.idUsuario,
      req.params.rideId,
      req.body
    );
    ...
    console.error('===== ERROR OFFER RIDE =====');
    console.error(error);
    console.error(error.message);
    console.error(error.stack);
...
export async function cancelRide(req, res) {
  try {
    ...
    console.error('===== ERROR CANCEL RIDE =====');
    console.error(error);
    console.error(error.message);
    console.error(error.stack);
